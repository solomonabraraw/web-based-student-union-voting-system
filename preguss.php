...
<?php	
$result = mysql_query("SELECT * FROM preguesstime");
$numrow = mysql_num_rows($result);
if ($numrow == 0)
{
	die('Now The Preguess DATE iS not Started!!!');
}
	...
	if($row1>0)
	{
				...
				{	
					...
					<table cellspacing="0">
						...
						<tr><td colspan="2"><a href="preguss.php?voteid=<?php echo $userid;?>" class="btn">Predict<a/></td></tr>
					</table>
					</div>
                    <?php
					}
			...
			if(mysql_num_rows($results)>0)
			{
				while ($row = mysql_fetch_array($results)) 
					...
					$sqlquer2=mysql_query("select * from requesttablepreguess WHERE Student_ID='$requestid2'");
					...
					$sqlquery=mysql_query("select * from requesttablepreguess WHERE Student_ID='$requestid'");
					while($result1=mysql_fetch_array($sqlquery))
					{
					$votestatus=$result1['votestatus'];	
					if($votestatus=='unvote')
					{
					$sql=mysql_query("select * from preguesstable where CID='$varvoteid'");
					...
					$VOICE=$row['VOICE'];
					$total=$VOICE+1;
					mysql_query("update preguesstable set VOICE='$total' where CID='$varvoteid'");
					//mysql_query("INSERT INTO `ballotstoretable` VALUES ( '$id','$varvoteid')");
					mysql_query("update requesttablepreguess set votestatus='vote' where Student_ID='$requestid'");
					echo"You have sucssusfully voted Thank you!";
					}
					else
					{
					mysql_query("INSERT INTO `preguesstable` VALUES ('$varvoteid','$fn $ln','$age','$sex','$collage','$dept','$yr','$cgpa','1')");	
					//mysql_query("INSERT INTO `ballotstoretable` VALUES ('$id','$varvoteid')");	
					mysql_query("update requesttablepreguess set votestatus='vote' where Student_ID='$requestid'");
					echo"You have sucssusfully Guess Thank you!";
					}
					}
					if($votestatus=='vote')
					{
				echo"<script> alert('Sorry!!! You are already Predict Before! you cannot vote again please!!!'); 
